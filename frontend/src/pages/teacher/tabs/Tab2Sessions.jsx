...
import { ClipboardList, Plus, Edit2, Trash2, Play, X, Save } from 'lucide-react'
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f8f9fa' }}>
                  {['#', '名称', '年级', '大单元', '小节', '时长', '题数', '状态', '操作'].map(h => (
              ...
              <tbody>
                {sessions.map((s, i) => {
                  const status = s.is_visible
                    ...
                    ? { label: `已完成(${s.submission_count})`, color: '#667eea', bg: '#e8eafc' }
                    : { label: '未开始', color: '#888', bg: '#f0f0f0' }
                  return (
                    <tr key={s.id} style={{ borderBottom: '1px solid #f0f0f0' }}
                      ...
                      <td style={{ padding: '10px 14px', whiteSpace: 'nowrap' }}>
                        ...
                        <button onClick={() => openEditSession(s)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#17a2b8', padding: '4px', display: 'inline-flex', marginLeft: 6 }}><Edit2 size={14} /></button>
                        <button onClick={() => deleteSession(s)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#dc3545', padding: '4px', display: 'inline-flex', marginLeft: 2 }}><Trash2 size={14} /></button>
                      </td>
                    </tr>
                  )
                })}
...
function SessionModal({ sModal, setSModal, sForm, setSForm, sMsg, handleSaveSession, sSaving, unitGradeFilter }) {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/api/admin/info/units/?grade=${encodeURIComponent(grade)}`, {
        ...
        <form onSubmit={handleSaveSession} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              ...
              <select value={sForm.grade} onChange={e => setSForm(p => ({ ...p, grade: e.target.value, units: [] }))}
